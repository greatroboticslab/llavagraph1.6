#!/usr/bin/env python3
"""
Evaluate ViT V2 model on official test set.
Uses: /data/ilminur/llavagraph1.6-master/data/test
"""
...
model_path = os.path.join(base_dir, "training_VIT/V2/vit_output_data_official")
test_dir = os.path.join(base_dir, "data/test")
output_dir = os.path.join(base_dir, "training_VIT/V2")

def main():
    print("=" * 70)
    print("  ViT V2 Model Evaluation (Official Data)")
    print("=" * 70)
    
    if not os.path.exists(model_path):
        print(f"Error: Model not found at {model_path}")
        print("Please train the model first using train_vit_official_data.py")
        sys.exit(1)
    ...
    print(f"\n✅ Overall Top-1 Accuracy: {accuracy:.2f}%")
    ...
    for i, cls in enumerate(class_names):
        ...
        if mask.sum() > 0:
            ...
            print(f"  {cls:12s}: {cls_acc:6.2f}% (P: {prec:.1f}%, R: {rec:.1f}%, F1: {f1:.1f}%)")
    ...
    metrics_path = os.path.join(output_dir, "vit_metrics_official_test.json")
    ...
    ax.set_title('ViT V2 - Confusion Matrix (Official Test Set)', fontsize=14)
    plt.tight_layout()
    
    cm_path = os.path.join(output_dir, "vit_confusion_matrix_official.png")
