#!/usr/bin/env python3
"""
train_mamba.py  —  MambaVision training on closed-loop data.

Usage:
    python train_mamba.py
    python train_mamba.py --model_id nvidia/MambaVision-S-1K
    python train_mamba.py --epochs 30 --batch_size 32
"""
...
# ── Path setup: reuse mambavision modules with closed-loop config ─────────────
HERE   = os.path.dirname(os.path.abspath(__file__))
MAMBA  = os.path.join(os.path.dirname(HERE), "mambavision")
sys.path.insert(0, HERE)    # closed_loop/config.py takes priority
sys.path.insert(1, MAMBA)   # mambavision/{model,dataset}.py

import config as cfg
# Monkey-patch the mambavision config module so dataset.py picks up our paths
import importlib, types
mamba_cfg = importlib.import_module("config")   # already loaded as cfg
for attr in dir(cfg):
...
def setup_logger(path):
    ...
    log = logging.getLogger("mamba_cl")
...
def train(args):
    ...
    log.info("=" * 60)
    log.info(f"  MambaVision — Closed-Loop Training")
    ...
    # Update config batch size so dataloaders pick it up
    cfg.BATCH_SIZE = args.batch_size

    loaders = build_dataloaders(cfg.DATA_DIR)
    class_weights = compute_class_weights(loaders["train"].dataset.labels).to(args.device)
    ...
    for epoch in range(args.epochs):
        t0 = time.time()
        tr = run_epoch(model, loaders["train"], criterion, optimizer, args.device, "train")
        vl = run_epoch(model, loaders["val"],   criterion, None,      args.device, "val")
