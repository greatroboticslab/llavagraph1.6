...
infer_falcon.py
===============
Few-shot inference with tiiuae/falcon-mamba-7b-instruct (pure Mamba, 7B).
...
def get_wtype(ex):
    src = ex.get("waveform") or ex.get("input", "")
    m = re.search(r"COMMANDED WAVEFORM: (\w+)", src)
...
def run_fewshot(ex):
    ...
    base_prompt = build_prompt(ex, shots)
    llm_text    = ""

    for header in ["DIAGNOSIS:", "\n\nCORRECTION:"]:
        current_prompt = base_prompt + llm_text + header
        input_ids = tokenizer(
            current_prompt, return_tensors="pt", add_special_tokens=False
        ).input_ids.to(model.device)

        print(f"  [{header.strip()}] input tokens: {input_ids.shape[1]}")

        with torch.no_grad():
            output_ids = model.generate(
                input_ids,
                max_new_tokens=MAX_PER_SECTION,
        ...
        stops = ["CORRECTION VECTOR:", "CORRECTION:", "User:", "<|im_end|>", "\n\n\n"]
        if header.strip() == "CORRECTION:":
            stops = ["CORRECTION VECTOR:", "User:", "<|im_end|>", "\n\n\n"]
        new_text = trim_at_stop(new_text, stops)
        llm_text += header + new_text
...
# ── run on first NUM_TEST test samples ───────────────────────────────────────

for i in range(NUM_TEST):
    wtype = get_wtype(test_data[i])
    print(f"\n{'='*65}")
    print(f"SAMPLE {i}  |  waveform: {wtype}")
    print(f"{'='*65}")
    result = run_fewshot(test_data[i])
    print("--- GENERATED ---")
    print(result)
    print("\n--- REFERENCE ---")
    print(test_data[i]["output"][:700])
